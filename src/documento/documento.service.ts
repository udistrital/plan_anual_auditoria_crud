...
export class DocumentoService {
  ...
  async put(id: string, DocumentoDTO: DocumentoDTO): Promise<Documento> {
    ...
    // await this.checkRelated(DocumentoDTO);
    const update = await this.DocumentoModel.findByIdAndUpdate(
      id,
      DocumentoDTO,
      { new: true },
    ).exec();
  ...
  async delete(id: string): Promise<Documento> {
    const deleted = await this.DocumentoModel.findByIdAndUpdate(
      id,
      { activo: false },
      { new: true },
    ).exec();
  ...
  async count(filterDto: FilterDto): Promise<number> {
    ...
    return await this.DocumentoModel.countDocuments(
      filtersService.getQuery(),
    ).exec();
  }
}
