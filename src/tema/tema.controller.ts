...
export class TemaController {
  ...
  @ApiResponse({ status: 400, description: 'Solicitud incorrecta.' })
  @ApiResponse({ status: 404, description: 'Tema no encontrado.' })
  async put(@Res() res, @Param('id') id: string, @Body() TemaDTO: TemaDTO) {
    try {
  ...
  async actualizarSubtema(
    ...
    try {
      const tema = await this.temaService.actualizarSubtema(
        temaId,
        subtemaId,
        SubtemaDTO,
      );
  ...
  async eliminarHallazgo(
    ...
    try {
      const tema = await this.temaService.eliminarHallazgo(
        temaId,
        subtemaId,
        hallazgoId,
      );
