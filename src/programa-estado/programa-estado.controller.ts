...
@ApiTags('programa-estado')
@Controller('programa-estado')
export class ProgramaEstadoController {
  constructor(private programaEstadoService: ProgramaEstadoService) {}
  @Post()
  @ApiOperation({ summary: 'Crear un nuevo estado de auditoria' })
  @ApiBody({ type: ProgramaEstadoDto })
  @ApiResponse({
    status: 201,
    description: 'El estado de auditoria ha sido creado exitosamente.',
    type: ProgramaEstadoDto,
  })
  @ApiResponse({ status: 400, description: 'Solicitud incorrecta.' })
  async post(@Res() res, @Body() programaEstadoDto: ProgramaEstadoDto) {
    try {
      const estadoPlan =
        await this.programaEstadoService.post(programaEstadoDto);
  ...
  async getAll(@Res() res, @Query() filterDto: FilterDto) {
    try {
      const estadoPrograma =
        await this.programaEstadoService.getAll(filterDto);
      const counts = await this.programaEstadoService.count(filterDto);
  ...
  async getById(@Res() res, @Param('id') id: string) {
    try {
      const estadoPlan = await this.programaEstadoService.getById(id);
      res.status(HttpStatus.OK).json({
        Success: true,
        Status: HttpStatus.OK,
        Message: 'Peticion Exitosa',
  ...
  async put(
    @Res() res,
    @Param('id') id: string,
    @Body() ProgramaEstadoDto: ProgramaEstadoDto,
  ) {
    try {
      const estadoPlan = await this.programaEstadoService.put(
        id,
        ProgramaEstadoDto,
      );
      res.status(HttpStatus.OK).json({
  ...
  async delete(@Res() res, @Param('id') id: string) {
    try {
      await this.programaEstadoService.delete(id);
      res.status(HttpStatus.OK).json({
        Success: true,
        Status: HttpStatus.OK,
