...
export class InformeController {
  ...
  @Get('/:informeId/tema')
  @ApiOperation({
    summary: 'Obtener temas, subtemas y hallazgos activos de un informe',
  })
  @ApiParam({
    name: 'informeId',
    type: 'string',
  ...
  @ApiResponse({
    status: 200,
    description:
      'Devuelve los temas activos con sus subtemas y hallazgos activos.',
  })
  @ApiResponse({ status: 404, description: 'Informe no encontrado.' })
  async getTemasActivos(@Res() res, @Param('informeId') informeId: string) {
    try {
      const temas =
        await this.informeService.getTemasActivosByInforme(informeId);
      res.status(HttpStatus.OK).json({
        Success: true,
        Status: HttpStatus.OK,
        Message: 'Peticion Exitosa',
  ...
  @Get('/:id/hallazgos')
  @ApiOperation({
    summary: 'Obtener todos los hallazgos de un informe (estructura plana)',
  })
  @ApiParam({ name: 'id', type: 'string', description: 'ID del informe' })
  @ApiResponse({
    status: 200,
    description:
      'Devuelve todos los hallazgos del informe en estructura plana.',
  })
  @ApiResponse({
    status: 404,
    description: 'Informe no encontrado o sin hallazgos.',
  })
  async getHallazgos(@Res() res, @Param('id') id: string) {
    try {
  ...
  async getEstados(@Res() res, @Param('id') id: string) {
    try {
  ...
  async getEstadoActual(@Res() res, @Param('id') id: string) {
    try {
      await this.informeService.getById(id);
